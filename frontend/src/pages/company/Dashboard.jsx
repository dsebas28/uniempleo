...
const APP_STATUS_MAP = { sent: 'bg-blue-100 text-blue-700', reviewing: 'bg-yellow-100 text-yellow-700', preselected: 'bg-purple-100 text-purple-700', interview: 'bg-emerald-100 text-emerald-700', selected: 'bg-emerald-100 text-emerald-800 font-bold', rejected: 'bg-red-100 text-red-600' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: 'Outfit' }}>
            Hola, {user?.profile?.name || user?.profile?.companyName || user?.email?.split('@')[0] || 'Empresa'} 👋
          </h1>
          <p className="text-slate-500 text-sm mt-0.5">Panel de gestión de vacantes y candidatos.</p>
        </div>
        <Link to="/empresa/vacantes/nueva" className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white rounded-xl bg-blue-600 hover:bg-blue-700 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="font-semibold text-gray-900 mb-5">Postulaciones por estado</h2>
          {chartData.length > 0 ? (
            ...
            <div className="h-48 flex items-center justify-center text-gray-400 text-sm">
              ...
              <Link to="/empresa/vacantes/nueva" className="text-blue-600 ml-1 hover:underline">Publica tu primera vacante</Link>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/empresa/candidatos" className="text-xs text-blue-600 hover:text-blue-700 flex items-center gap-1">
              Ver todas <ArrowRight size={12} />
            </Link>
          </div>

          {recentApps.length === 0 ? (
            <div className="text-center py-8 text-gray-400 text-sm">Aún no tienes postulaciones</div>
          ) : (
            <div className="space-y-3">
              {recentApps.slice(0, 5).map(app => {
                const sc = APP_STATUS_MAP[app.status] || 'bg-gray-100 text-gray-600';
                return (
                  <div key={app.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">{app.student_name}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          ...
          >
            ...
            <ArrowRight size={16} className="text-gray-300 ml-auto group-hover:text-blue-500 transition-colors" />
          </Link>
        ))}
      </div>
    </div>
